        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 420px), 1fr))', gap: '1px', background: '#0f0f0f' }}>
          {packages.map(p => (
            <div key={p.name} style={{ background: '#000', padding: '2.5rem' }}>
              ...
              <p style={{ color: '#aaaaaa', fontSize: '0.88rem', lineHeight: 1.6, marginBottom: '1.5rem' }}>{p.desc}</p>
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 2rem 0' }}>
                {p.includes.map(item => (
                  <li key={item} style={{ color: '#b8b8b8', fontSize: '0.85rem', padding: '0.35rem 0', borderBottom: '1px solid #0a0a0a', display: 'flex', gap: '0.5rem' }}>
                    <span style={{ color: '#9a9a9a' }}>—</span> {item}
                  </li>
                ))}
              </ul>
